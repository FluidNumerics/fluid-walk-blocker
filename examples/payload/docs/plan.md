...
because a preempted walk that a `REQUEUE` policy puts back on the queue
walks the filesystem again. The bound moves from depth
...
service group may hold write on a spool ancestor, never on the spool, and
root's writes into the spool never follow a link. Hooks are verified, not assumed (ADR-0008):
